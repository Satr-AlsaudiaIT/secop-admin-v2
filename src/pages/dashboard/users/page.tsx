    ...
    <>
      ...
      <MainTable
        ...
        cols={generateCols([
          ...
          {
            title: "status",
            content: (_, record) => statusList[record.status],
          },
          {
            title: "employee_id_image",
            content: (_, record) => <Image width={100} src={record.image} />,
          },
          ...(function () {
            const show =
                      ...
                      <div className="flex gap-2">
                        {record.status === 0 && (
                          <Tooltip title={<FormattedMessage id="change_status" />}>
                            <Button onClick={() => showStatusModal(record)} icon={<EditOutlined key="edit" />} />
                          </Tooltip>
                        )}
                      </div>
                    ),
